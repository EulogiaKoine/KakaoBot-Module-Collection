...
function responseFix(room, msg, sender, isGroupChat, replier, imageDB, roomId){
var Game = Module_Game.load();
if(msg === "!htest")
    replier.reply("success")
try{
if(msg.slice(0, 2).toLowerCase() !== 'hl') return;
const cmd = msg.split(" ");
...
if((sender === "코이네" || sender.trim() === "「 점장 」 Regret ; 그렛") && msg.startsWith("hle ")){
    try{replier.reply(eval(msg.substr(3).trim()));}catch(e){replier.reply(e+"\n"+e.lineNumber);}
    return
}
...
if(!["사용","규칙","도움말","게임","직업","정보","배경","투표","시간","프로필","사화","밀회","설파","판결","플","플레이어","회고"].includes(cmd[1])){
  if(cmd[0] == "hle") return;
...
if(cmd[1] == "게임"){
    switch(cmd[2]){
        ...
        case "참가":
            ...
            PlayerList.add(sender,code);
            replier.reply(prefix+"• 게임에 참가하였습니다. 봇 개인톡에 현재 프로필로 아무 채팅을 남겨주세요.");
            return;
        ...
        case "시작":
            ...
            if(Game.gaming){
                ...
                return;
            } else if(Game.players.some(v=>!SM.hasSendSession(v))){
                replier.reply(prefix+"- 개인톡으로 안내메세지 전송이 불가능한 플레이어가 있습니다. 확인해주세요."
                    +"\n\n < 대상 >\n" + Game.players.filter(v => !SM.hasSendSession(v)).join('\n'));
                return;
            }
            dead_talk_log = ''
...
if(cmd[1] === "회고"){
    if(Game.gaming)
        replier.reply(prefix + "- 플레이 중에는 불가능합니다.")
    else if(dead_talk_log === '')
        replier.reply(prefix + "- 기록이 없습니다.")
    else
        replier.reply(prefix +
            + "(*영혼의 기억을 회고하려면 클릭하세요)" + "\u200b".repeat(500)
            + "\n\n\n       ⏳\n" + dead_talk_log)
    return
}
...
if(cmd[1] == "투표"){
    ...
    if(Player.process.voting.length >= 1){
        for(let i=0; i<Player.process.voting.length; i++){
...
if(cmd[1] == "사화"){
    ...
    dead_talk_log += '\n(영혼)' + sender + msg
    deads.forEach(v => PlayerList.player(v).hear("(영혼)"+sender,msg));
    return;
}
...
const timer = setInterval(function(){
    let Game = Module_Game.load();
    let PlayerList = Module_PlayerList.load();
    let Events = Module_Event.load();

    if(Events.timers.length === 0) return;
    let now = parseInt(new Date().getTime()/1000);
    const timers = Events.timers.splice(0, Events.timers.length)
    for(let timer of timers){
        if(timer.end-30 == now)
            Game.announce(timer.name === "대기"? "방 자동 삭제까지 앞으로 30초.": timer.name.replace("_"," ")+"까지 30초 남았습니다.");
        else if(timer.end-60 == now)
            Game.announce(timer.name === "대기"? "방 자동 삭제까지 앞으로 1분": timer.name.replace("_"," ")+"까지 1분 남았습니다.")
        else if(timer.end <= now){
            if(!java.io.File(sdcard+'/Leader_of_History/1.5/Process/Events/'+timer.name+'.js')){
                Game.announce("이벤트 "+timer.name+"의 프로세스를 찾을 수 없어 실행되지 않습니다.");
                runs.shift();
                continue;
            }
            let process = require(sdcard+'/Leader_of_History/1.5/Process/Events/'+timer.name+'.js');
            try{
                process(Game,PlayerList,Events);
            }catch(e){
                Game.announce("이벤트 오류 발생 : "+e+"\n• 발생 줄 : "+e.lineNumber);
            }finally{
                Events.delete(timer.name);
            }
        }
    }
},1000);
...
const SM = inspire('ui.SessionManager')
const UIManager = require('UIManager')
const onNotificationPosted = UIManager.setUIEnvironment(SM, Api, responseFix)

// Api.replyRoom("프로젝트 - 테스팅 방★", "session test")