...
/**
 * @author Koinē
 * @version 1.2 2023-10-04, notification form interpretation updated
 * 
...
function bindSession(pack, room, acts){
    if(!Companion.hasSession(pack, room) && acts.some(v => v.getRemoteInputs())){
        Companion.setSession(pack, room, acts.find(v => v.getRemoteInputs()))
        Companion.setMarkAsRead(pack, room, acts.find(v => v.getRemoteInputs() === null))
    }
}
